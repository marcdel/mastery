defmodule Mastery.Core.QuestionTest do
  ...
  test "function generators are called" do
    generations = addition_generators(fn -> 42 end, [0])
    substitutions = build_question(generators: generations).substitutions
    assert Keyword.fetch!(substitutions, :left) == generations.left.()
  end
  ...
  test "alternative: a random choice is made from list generators" do
    generators = addition_generators(Enum.to_list(1..9), [0])
